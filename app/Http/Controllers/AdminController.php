...
class AdminController extends Controller
{
    public function index()
    {
        ...
        // 2. Total Semua Quotation (Karena tidak ada status 'acc', kita hitung semua yang ada)
        $total_acc = DB::table('quotation')->count();

        // 3. Total Omset / Penjualan dari Seluruh Quotation yang Ada
        $total_sales = DB::table('quotation')->sum('total_harga') ?? 0;

        // 4. Data Dummy untuk Grafik Lingkaran (Dihapus kuerinya agar tidak error)
        // Set nilai 0 agar jika di halaman view admin.index masih memanggil variabel ini, program tidak crash.
        $status_data = ['draft' => 0, 'sent' => 0, 'acc' => 0, 'rejected' => 0];
        ...
        // 6. Navbar Notifications (4 Quotation terbaru)
        // Menghilangkan klausa where status, langsung menarik rincian quotation terbaru
        ...
        // 7. Data untuk Grafik Garis Penjualan Berdasarkan Tanggal (Flot Chart)
        // Menghilangkan pengetatan status 'acc'
        ...
        // Di-set NULL pada bagian desain jika tabel quotation Anda tidak memuat kolom berkas desain
        $sub_query = DB::table('detail_quotation as dq')
            ->join('quotation as q', 'dq.id_quotation', '=', 'q.id_quotation')
            ->select('dq.id_produk', DB::raw('NULL as desain'), 'dq.id_detail');
        ...
        // Mengirimkan semua data ke view Blade di dalam folder admin
